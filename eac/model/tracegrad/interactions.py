...
class Equi_Nonlin_Grad_Module(torch.nn.Module):
    def __init__(self, irreps_in, z_fea_len, hidden_fea_len=1024):
        ...
        self.nonlinear_layers = torch.nn.Sequential(
            torch.nn.Linear(hidden_fea_len, hidden_fea_len),
    ...
    def forward(self, tensor_in,retain_graph=False):
...
def get_gate(
    ...
    irreps_out = irreps_in = gate.irreps_out
    newgate = Equi_Nonlin_Grad_Module(irreps_in, 49)
    # irreps_in: o3.Irreps = gate.irreps_in
    # irreps_out: o3.Irreps = gate.irreps_out
    return newgate, irreps_in, irreps_out
